#!/usr/bin/env python3
"""Insert <img class="scam-comic"> tags into Costa Rica scam pages for the first time.

After the v2 pipeline generates comics for all 8 Costa Rica cities and uploads
...
`<img class="scam-comic">` goes directly after `<div class="scam-location">...`,
before `<p class="scam-tldr">` / `<p class="scam-story-body">`.

Idempotent: skips any scam-card that already has an img.scam-comic tag, and
any scam flagged as failed in /tmp/costa-rica-audit.jsonl.

Usage:
    python3 scripts/comic-pipeline/insert_costa_rica_comics.py
"""
...
AUDIT_LOG = Path("/tmp/costa-rica-audit.jsonl")
