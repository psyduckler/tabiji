...
for (const def of regionDefinitions) {
  ...
  writePage(`compare/${def.slug}/index.html`, pageHtml({
    ...
    sections: [
      `<section class="section"><div class="shell"><div class="section-header"><div><h2>Most popular ${safeText(def.label)} comparisons</h2><p>These are the strongest candidates for internal linking, navigation modules, and future editorial support.</p></div></div><div class="mini-grid">${top.slice(0,6).map(renderMiniCard).join('')}</div></div></section>`,
      `<section class="section"><div class="shell"><div class="section-header"><div><h2>Newest in ${safeText(def.label)}</h2><p>Fresh pages in this region stay visible without taking over the whole compare homepage.</p></div></div><div class="mini-grid">${sortByNewest(items).slice(0,6).map(renderMiniCard).join('')}</div></div></section>`
    ],
    archiveCards: top,
