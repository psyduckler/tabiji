...
// Country/destination cluster hubs — these aren't generated by this script
// (they live under expand_cluster_hubs.py with a different template), but the
// master compare hub needs to surface them as browse entries so they're not
// orphaned from /compare/.
...
function pageHtml({ title, metaDescription, canonicalPath, heroKicker, heroTitle, heroText, heroStats, sidebarTitle, sidebarItems, sections, archiveCards, archivePrefillRegion = '', archivePrefillType = '', introCards = [], externalDataUrl = null }) {
  ...
  const faqItems = [
    ...
    },
    {
      ...
      answer: 'Use the top and newest sections as a quick shortlist, then use the ranked archive filters to narrow by region, trip type, or destination keywords.'
    }
  ];
...
function homepageSections(cards) {
  ...
  const countryGroups = countryDefinitions.map(def => ({ ...def, items: cards.filter(card => card.cluster === def.slug) })).filter(group => group.items.length);
  return {
    sections: [
      ...
      `<section class="section"><div class="shell"><div class="section-header"><div><h2>Browse by destination</h2><p>Country and island-cluster hubs for travelers who already know roughly where they want to go.</p></div></div><div class="browse-grid">${countryGroups.map(group => renderBrowseGroup(group, '/compare')).join('')}</div></div></section>`,
