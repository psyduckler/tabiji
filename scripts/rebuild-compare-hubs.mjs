...
const sharedHeadHtml = fs.readFileSync(path.join(repoRoot, '_includes', 'shared-head.html'), 'utf8').trim();
const footerHtml = fs.readFileSync(path.join(repoRoot, '_includes', 'footer-default.html'), 'utf8').trim();
...
function renderArchiveScript(cardsForPage, searchOptions = { region: true, type: true, paginate: false, externalDataUrl: null }) {
  ...
  if (searchOptions.externalDataUrl) {
    ...
    function renderPage() {
      ...
      resultCount.textContent = 'Showing ' + visible.length.toLocaleString('en-US') + ' of ' + filtered.length.toLocaleString('en-US') + ' comparison' + (filtered.length === 1 ? '' : 's');
  ...
  return `<script>
    ...
    function applyFilters() {
      ...
      filtered = filtered.sort((a,b) => sort === 'az' ? a.title.localeCompare(b.title) : sort === 'newest' ? ((b.updatedAt||'').localeCompare(a.updatedAt||'') || b.popularityScore - a.popularityScore) : (b.popularityScore - a.popularityScore || (b.updatedAt||'').localeCompare(a.updatedAt||'')));
      resultCount.textContent = 'Showing ' + filtered.length.toLocaleString('en-US') + ' comparison' + (filtered.length === 1 ? '' : 's');
...
function pageHtml({ title, metaDescription, canonicalPath, heroKicker, heroTitle, heroText, heroStats, sidebarTitle, sidebarItems, sections, archiveCards, archivePrefillRegion = '', archivePrefillType = '', introCards = [], externalDataUrl = null }) {
  ...
  if (canonicalPath !== '/compare/') {
    breadcrumbItems.push({ '@type': 'ListItem', position: 3, name: heroTitle, item: canonicalUrl });
  }
  const faqItems = [
    {
...
const roundedCount = `${(Math.floor(cards.length / 100) * 100).toLocaleString('en-US')}+`;
writePage('compare/index.html', pageHtml({
  title: `Travel Comparisons | Compare ${roundedCount} Destinations Side by Side | Tabiji`,
  metaDescription: `Compare ${roundedCount} destinations side by side with Tabiji. Real costs, weather, safety, and honest traveler verdicts — grouped by region and trip type, and ranked by what travelers actually search for.`,
  canonicalPath: '/compare/',
  heroKicker: 'Compare destinations faster',
  heroTitle: `Compare ${roundedCount} destinations, side by side.`,
  heroText: 'Real costs, weather, safety, and honest traveler verdicts — grouped by region and trip type, and ranked by what travelers actually search for.',
  heroStats: [{ value: cards.length.toLocaleString('en-US'), label: 'published comparisons' }, { value: String(regionDefinitions.length), label: 'regional hubs' }, { value: String(typeDefinitions.length), label: 'trip-type hubs' }],
