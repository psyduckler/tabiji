...
def slugify(name: str) -> str:
    # Transliterate accented chars (ö→o, å→a, é→e, ñ→n, etc.) before stripping
...
def main() -> None:
    ...
    for raw in TSV.strip().splitlines():
        ...
        seen.add(slug)
    if len(additions) < 490:
        raise SystemExit(f'Only {len(additions)} unique additions after filtering; need at least 490')
