...
OUTPUT_SNAPSHOT = ROOT / 'scripts' / 'data' / 'next-2000-destinations-source.json'
TARGET_ADD = 2000
...
def season_for(continent: str, timezone: str, country_code: str) -> str:
    if continent == 'Europe':
        return 'Apr–Oct'
    if continent == 'North America':
        return 'Apr–Oct'
    if continent == 'Asia':
        if country_code in {'AE','BH','IQ','IR','JO','KW','LB','OM','QA','SA'}:
            return 'Oct–Apr'
        return 'Oct–Apr'
    if continent == 'Africa':
        return 'Oct–Apr'
...
def pitch_for(name: str, population: int) -> str:
    template = CITY_PITCHES[population % len(CITY_PITCHES)]
...
def main() -> None:
    ...
    for record in sorted(iter_geonames(), key=lambda r: (-r['population'], norm_text(r['asciiname'] or r['name']), r['geonameid'])):
        if record['feature_class'] != 'P':
            continue
        if record['feature_code'] not in {'PPLC', 'PPLA', 'PPLA2'}:
            continue
        ...
        addition = {
            ...
            'season': season_for(continent, record['timezone'], record['country_code']),
