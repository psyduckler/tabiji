#!/usr/bin/env python3
"""
Build Kindle EPUB from JSON scam data + manuscript markdown.

Usage:
    python3 book/build.py

Produces: book/build/japan-scams.epub
Requires: pandoc, pyyaml
"""
from __future__ import annotations

import json
import re
import subprocess
import sys
from pathlib import Path

try:
    import yaml
except ImportError:
    sys.exit("pyyaml is required: pip3 install pyyaml")

HERE = Path(__file__).parent
CONFIG = yaml.safe_load((HERE / "config.yaml").read_text())
MANUSCRIPT = HERE / "manuscript"
ASSETS = HERE / "assets"
TEMPLATES = HERE / "templates"
BUILD = HERE / CONFIG.get("output_dir", "build")
BUILD.mkdir(exist_ok=True)

CITIES = CONFIG["cities"]
DATA_DIR = (HERE / CONFIG["scam_data_dir"]).resolve()

CITY_INSERTION_MARKER = "<!-- CITIES -->"

# R2 fallback — when a local asset is absent, fall back to the same path
# served from img.tabiji.ai. Mirror keys; pandoc fetches HTTPS images and
# embeds them in the EPUB. Necessary because the local JPGs are scheduled
# for git-rm in a follow-up commit; without this fallback the EPUB build
# would silently produce an imageless book.
R2_BASE = "https://img.tabiji.ai"


def _img_ref(rel: str) -> str:
    """Return a markdown-image ref for `rel` (relative to HERE).

    If the local file exists, use the relative path so pandoc dedupes the
    image inside the EPUB. Otherwise fall back to the R2 mirror URL —
    pandoc fetches HTTPS images and embeds them in the EPUB.
    """
    if (HERE / rel).exists():
        return rel
    return f"{R2_BASE}/{HERE.name}/{rel}"


def _rewrite_md_assets(md: str) -> str:
    """Rewrite `](assets/...jpg)` refs in static manuscript markdown.

    Static .md files (e.g. 04-cities-section.md) hard-code relative paths
    to assets/cities/X.jpg. After PR 2 those paths won't resolve on disk,
    so substitute the R2 URL when the local file is absent.
    """
    def repl(m: re.Match) -> str:
        rel = m.group(1)
        return f"]({_img_ref(rel)})"
    return re.sub(r"\]\((assets/[a-z0-9/_-]+\.jpg)\)", repl, md)

# Rich alt text for city chapter openers (screen-reader friendly).
CITY_ALT_TEXT: dict[str, str] = {
    # Greece volume
    "athens": "Athens — the Parthenon atop the Acropolis at golden hour",
    "santorini": "Santorini — blue-domed churches of Oia perched on the caldera cliff",
    "mykonos": "Mykonos — the famous windmills at Little Venice at dusk",
    "thessaloniki": "Thessaloniki — the White Tower on the waterfront promenade",
    "rhodes": "Rhodes — medieval walls and the Palace of the Grand Master",
    "corfu": "Corfu — the Liston arcade with the Old Fortress in the distance",
    "heraklion": "Heraklion — the Venetian fortress of Koules guarding the harbour",
    "chania": "Chania — the Venetian Lighthouse and Old Harbour at dusk",
    "paros": "Paros — whitewashed houses of Naoussa fishing village at sunset",
    "naxos": "Naxos — the Portara gateway to the Temple of Apollo at sunset",
    "meteora": "Meteora — clifftop monasteries balanced on sandstone pinnacles at sunrise",
    "delphi": "Delphi — Tholos rotunda of Athena Pronaia on Mount Parnassus",
    "nafplio": "Nafplio — the Bourtzi sea-fortress and the harbor below Palamidi",
    "kos": "Kos — the Asclepieion ancient healing sanctuary on a hillside",
    "zakynthos": "Zakynthos — Navagio Shipwreck Beach in the cliff-ringed cove",
    "milos": "Milos — the white-rock lunar landscape of Sarakiniko at sunset",
}


def load_city(slug: str) -> dict:
    path = DATA_DIR / f"{slug}.json"
    if not path.exists():
        raise FileNotFoundError(f"{path} not found")
    return json.loads(path.read_text())


def scam_md(scam: dict, image_ref: str | None = None) -> str:
    cat = scam["category"].title()
    sev = scam["severity"].title()
    freq = scam["frequency"].title()
    parts = [
        f'## {scam["name"]}\n\n',
        f"**{cat}** · Severity: {sev} · Frequency: {freq}\n\n",
    ]
    if image_ref:
        # Descriptive alt text for screen readers and KDP accessibility compliance.
        # Category + name + severity is what a blind reader needs to know before
        # the heading that follows explains the scam in detail.
        alt = (
            f'Stylized illustration depicting the {scam["name"]} scam — '
            f'a {cat.lower()} scam rated {sev.lower()} severity'
        )
        # Either a repo-relative path or an https:// URL; pandoc handles both.
        parts.append(f"![{alt}]({image_ref})\n\n")
    parts.extend([
        f'### How this scam works\n\n{scam["description"]}\n\n',
        f'### How to avoid it\n\n{scam["avoidance"]}\n\n',
        f'**Where it happens:** {scam["location"]}\n\n',
    ])
    if scam.get("tags"):
        parts.append(f'*{" · ".join(scam["tags"])}*\n\n')
    return "".join(parts)


def city_chapter_md(data: dict) -> str:
    city = data["city"]
    slug = data["slug"]
    parts = [f"\n\n# {city}\n\n"]
    # Chapter-opening city illustration (flat-vector travel-poster style).
    # Lives at assets/cities/<slug>.jpg — generated by scripts/gen_city_illustrations.py.
    # Uses the same relative path form as the gallery so pandoc dedupes the
    # image inside the EPUB (otherwise each of the 9 cities ships twice).
    alt = CITY_ALT_TEXT.get(slug, f"Stylized illustration of {city}")
    parts.append(f"![{alt}]({_img_ref(f'assets/cities/{slug}.jpg')})\n\n")
    intro_path = MANUSCRIPT / f"cities-{slug}-intro.md"
    if intro_path.exists():
        parts.append(intro_path.read_text().strip() + "\n\n")
    else:
        parts.append(
            f"> **TODO** — write a 1-page intro for {city}: "
            f"overall risk level, top 3 neighborhoods to watch, "
            f"nearest koban, one safe recommendation.\n\n"
        )
    parts.append(
        f'This chapter documents {len(data["scams"])} scams reported in {city}.\n\n'
    )
    # Greece uses scam-comics directory with numbered files (1.jpg, 2.jpg, etc.)
    for idx, scam in enumerate(data["scams"], start=1):
        parts.append(scam_md(scam, _img_ref(f"assets/scam-comics/{slug}/{idx}.jpg")))
    return "".join(parts)



def static_chapters() -> list[tuple[str, str]]:
    """Return numbered manuscript files sorted by filename."""
    files = sorted(MANUSCRIPT.glob("[0-9][0-9]-*.md"))
    return [(f.name, _rewrite_md_assets(f.read_text())) for f in files]


def assemble_markdown() -> str:
    parts: list[str] = []
    for fname, content in static_chapters():
        if CITY_INSERTION_MARKER in content:
            parts.append(content.replace(CITY_INSERTION_MARKER, ""))
            for c in CITIES:
                try:
                    parts.append(city_chapter_md(load_city(c)))
                except FileNotFoundError:
                    parts.append(
                        f"\n# {c.title()}\n\n"
                        f"> **TODO** — data missing for {c}\n\n"
                    )
        else:
            parts.append(content)
        parts.append("\n\n")
    return "".join(parts)


def build_epub(md: str) -> Path:
    md_path = BUILD / "manuscript.md"
    md_path.write_text(md)

    out_name = CONFIG.get("output_filename", "japan-scams")
    epub_path = BUILD / f"{out_name}.epub"
    cover = ASSETS / "cover.jpg"
    css = TEMPLATES / "style.css"

    cmd = [
        "pandoc",
        str(md_path),
        "-o",
        str(epub_path),
        "--resource-path",
        str(HERE),
        "--metadata",
        f'title={CONFIG["title"]}',
        "--metadata",
        f'subtitle={CONFIG["subtitle"]}',
        "--metadata",
        f'author={CONFIG["author"]}',
        "--metadata",
        f'publisher={CONFIG["publisher"]}',
        "--metadata",
        f'lang={CONFIG["language"]}',
        "--metadata",
        f'rights={CONFIG["rights"]}',
        "--metadata",
        f'description={CONFIG["description"].strip()}',
        "--toc",
        "--toc-depth=2",
        "--split-level=1",
    ]
    if cover.exists():
        cmd.extend(["--epub-cover-image", str(cover)])
    if css.exists():
        cmd.extend(["--css", str(css)])

    subprocess.run(cmd, check=True)
    return epub_path


def stats(md: str) -> dict:
    words = len(re.findall(r"\b\w+\b", md))
    todos = len(re.findall(r"\*\*TODO\*\*", md, re.IGNORECASE))
    scam_count = sum(len(load_city(c).get("scams", [])) for c in CITIES if (DATA_DIR / f"{c}.json").exists())
    return {"words": words, "todos": todos, "scams": scam_count}




def _inline_svg_images(svg_text: str, svg_dir: Path) -> str:
    """Inline every <image href> as a base64 data URI.

    rsvg-convert 2.62+ silently drops image refs (relative, absolute, AND
    HTTPS URLs) — without inlining we'd render a text-only cover. For
    HTTPS URLs pointing at the R2 mirror, prefer the local file if it
    still exists; otherwise fetch the bytes from the URL.
    """
    import base64
    import mimetypes
    import urllib.request

    repo_root = HERE.parent

    def _inline(match: re.Match) -> str:
        attr, quote, href = match.group(1), match.group(2), match.group(3)
        data: bytes | None = None
        mime: str | None = None
        if href.startswith(("http://", "https://")):
            # Try local mirror first (R2 keys are repo-relative paths).
            r2_prefix = f"{R2_BASE}/"
            if href.startswith(r2_prefix):
                local = repo_root / href[len(r2_prefix):]
                if local.exists():
                    data = local.read_bytes()
                    mime = mimetypes.guess_type(local.name)[0] or "image/jpeg"
            if data is None:
                try:
                    with urllib.request.urlopen(href, timeout=30) as r:
                        data = r.read()
                        mime = r.headers.get("Content-Type", "image/jpeg")
                except Exception:
                    return match.group(0)
        elif href.startswith("data:"):
            return match.group(0)
        else:
            local = (Path(href) if href.startswith("/") else svg_dir / href)
            if not local.exists():
                return match.group(0)
            data = local.read_bytes()
            mime = mimetypes.guess_type(local.name)[0] or "image/jpeg"
        encoded = base64.b64encode(data).decode("ascii")
        return f"{attr}={quote}data:{mime};base64,{encoded}{quote}"

    return re.sub(
        r"(xlink:href|href)=(['\"])([^'\"]+)\2",
        _inline,
        svg_text,
    )


def render_cover() -> None:
    """Rebuild cover.jpg from the SVG source if the SVG is newer.
    Produces a 1600x2560 JPG (KDP spec, 1.6:1 aspect, ~300 DPI)."""
    svg = ASSETS / "svg" / "front.svg"
    jpg = ASSETS / "cover.jpg"
    if not svg.exists():
        return
    if jpg.exists() and jpg.stat().st_mtime >= svg.stat().st_mtime:
        return
    # Inline all <image href>s as base64 — rsvg-convert 2.62+ silently drops
    # relative/absolute/HTTPS image refs, so without this the cover renders
    # as a text-only gradient.
    svg_inlined = _inline_svg_images(svg.read_text(), svg.parent)
    inlined_svg = BUILD / "cover_tmp.svg"
    inlined_svg.write_text(svg_inlined)
    png_tmp = BUILD / "cover_tmp.png"
    subprocess.run(
        ["rsvg-convert", "-w", "1600", "-h", "2560", str(inlined_svg), "-o", str(png_tmp)],
        check=True,
    )
    # Prefer ImageMagick if available; fall back to macOS sips (built-in).
    from shutil import which as _which
    if _which("magick"):
        subprocess.run(["magick", str(png_tmp), "-quality", "90", str(jpg)], check=True)
    elif _which("sips"):
        subprocess.run(
            ["sips", "-s", "format", "jpeg", "-s", "formatOptions", "90",
             str(png_tmp), "--out", str(jpg)],
            check=True, capture_output=True,
        )
    else:
        raise RuntimeError("Need either ImageMagick (magick) or macOS sips to convert PNG→JPG")
    png_tmp.unlink(missing_ok=True)
    inlined_svg.unlink(missing_ok=True)

def main() -> None:
    render_cover()
    md = assemble_markdown()
    epub_path = build_epub(md)
    s = stats(md)
    print(f"✓ Built: {epub_path}")
    print(f"  Size:  {epub_path.stat().st_size / 1024:.1f} KB")
    print(f"  Words: {s['words']:,}")
    print(f"  Scams: {s['scams']}")
    print(f"  TODOs remaining: {s['todos']}")


if __name__ == "__main__":
    main()
