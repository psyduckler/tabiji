#!/usr/bin/env python3
"""
Build the paperback-interior PDF for the Greece book.
...
INTERIOR_HTML = BUILD / "greece-scams-paperback.html"
INTERIOR_PDF = BUILD / "greece-scams-paperback.pdf"
...
/* Keep a scam H2 + category line + comic figure glued together so we never
   orphan a scam title at the bottom of a page. */
...
/* The italic tag-only paragraph at the end of each scam entry (e.g. "gem · fraud")
   should not be indented. Treat as a metadata line. */
...
/* ===== Emphasis — italic for Thai terms / book titles, consistent color ===== */
...
/* ===== Manual Contents table styling ===== */
h1#contents + table,
h1:contains("Contents") + table {
  width: 100%;
...
h1#contents + table td,
h1:contains("Contents") + table td {
  border: none;
  padding: 2pt 0;
  vertical-align: bottom;
}
h1#contents + table tr,
h1:contains("Contents") + table tr {
  border-bottom: 0.3pt dotted #aaa;
}
h1#contents + table td:first-child,
h1:contains("Contents") + table td:first-child {
  padding-right: 1em;
}
h1#contents + table td:last-child,
h1:contains("Contents") + table td:last-child {
  text-align: right;
  font-variant-numeric: tabular-nums;
  white-space: nowrap;
}
...
def build_html(md: str) -> Path:
    ...
    cmd = [
        ...
        # TOC disabled — we use a manually-created Contents page with page numbers
