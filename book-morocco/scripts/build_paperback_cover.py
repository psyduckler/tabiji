#!/usr/bin/env python3
"""
Build the KDP paperback wraparound cover PDF for the Turkey book.
...
The source SVGs in assets/svg/ were designed for a 500x800-unit viewBox;
they are uniformly scaled to fit the 9" trim height and centered horizontally
inside the 6" trim width — the side gap fills naturally with the warm bleed
gradient so the preserved 5:8 composition reads as an intentionally-generous
framed design rather than a letterbox.

Default --pages (358, cream) matches the final China interior at 6x9 trim
with cream paper, producing a spine of 0.895" and a total wraparound of
12.775" x 9.25" including the 0.125" bleed.

Usage:
    python3 book-spain/scripts/build_paperback_cover.py                 # 358 pages, cream
    python3 book-spain/scripts/build_paperback_cover.py --pages 360
    python3 book-spain/scripts/build_paperback_cover.py --paper white
"""
...
import sys, yaml  # noqa: E402
_CONFIG = yaml.safe_load((BOOK / "config.yaml").read_text())
_OUT = _CONFIG.get("output_filename", "morocco-scams").replace("-scams", "")
OUT_SVG = BOOK / "build" / f"{_OUT}-paperback-cover.svg"
OUT_PDF = BOOK / "build" / f"{_OUT}-paperback-cover.pdf"
...
def paperback_back_content(back_inner: str) -> str:
    ...
    - Relabel footer from KINDLE EDITION → PAPERBACK EDITION and nudge it
      upward so it does not compete with Amazon's auto-generated barcode
      zone (Amazon reserves roughly the lower-right 2" × 1.2" of the back
      cover). The Spain eBook back.svg does not include a QR panel, so we
      skip the QR-removal step used in earlier series volumes.
    """
    back_inner = re.sub(
...
def main() -> None:
    ...
    ap.add_argument("--pages", type=int, default=310,
                    help="Interior page count (default 358 cream = 0.895\" spine)")
    ...
    trim_w = TRIM_W_IN * UNITS_PER_IN
    trim_h = TRIM_H_IN * UNITS_PER_IN
    bleed = BLEED_IN * UNITS_PER_IN
    spine = spine_in * UNITS_PER_IN
  ...
  <!-- ================================================================ -->
  <!-- TURKEY PAPERBACK WRAPAROUND COVER                                 -->
  ...
  <!-- Full-canvas warm bleed (matches cover palette so any trim drift
       still produces a clean ochre edge, not a white stripe) -->
  <defs>
...
{back_inner}
  ...
  <!-- TABIJI mark near the top of the spine -->
