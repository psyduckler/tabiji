...
# served from img.tabiji.ai. Mirror keys; pandoc fetches HTTPS images and
# embeds them in the EPUB. Necessary because the local JPGs are scheduled
# for git-rm in a follow-up commit; without this fallback the EPUB build
# would silently produce an imageless book.
R2_BASE = "https://img.tabiji.ai"

# Set of repo-relative paths (under this book) that are mirrored to R2.
# Built from the migration manifest; only these paths fall back to R2
# when local is absent. References to non-mirrored paths (e.g. cities
# whose slug doesn't match a file) preserve the original "skip" behavior.
_R2_MIRRORED: set[str] = {
    "assets/cities/jaco.jpg",
    "assets/cities/la-fortuna.jpg",
    "assets/cities/liberia.jpg",
    "assets/cities/manuel-antonio.jpg",
    "assets/cities/monteverde.jpg",
    "assets/cities/puerto-viejo.jpg",
    "assets/cities/san-jose.jpg",
    "assets/cities/tamarindo.jpg",
    "assets/cover.jpg",
    "assets/svg/back.jpg",
    "assets/svg/front.jpg",
}


def _img_ref(rel: str) -> str | None:
    """Return a markdown-image ref for `rel` (relative to HERE), or None.

    If the local file exists, use the relative path so pandoc dedupes the
    image inside the EPUB. Otherwise, if the path is mirrored to R2, fall
    back to the R2 URL. Otherwise return None so the caller can skip.
    """
    if (HERE / rel).exists():
        return rel
    if rel in _R2_MIRRORED:
        return f"{R2_BASE}/{HERE.name}/{rel}"
    return None


def _rewrite_md_assets(md: str) -> str:
    """Rewrite `](assets/...jpg)` refs in static manuscript markdown.

    Static .md files (e.g. 04-cities-section.md) hard-code relative paths
    to assets/cities/X.jpg. After PR 2 those paths won't resolve on disk,
    so substitute the R2 URL when the local file is absent. Refs to paths
    not mirrored to R2 are left alone (they were already broken on disk).
    """
    def repl(m: re.Match) -> str:
        rel = m.group(1)
        ref = _img_ref(rel)
        return f"]({ref})" if ref is not None else m.group(0)
    return re.sub(r"\]\((assets/[a-z0-9/_-]+\.jpg)\)", repl, md)
...
def scam_md(scam: dict, image_path: Path | None = None) -> str:
    ...
    # Convert snake_case categories ("aggressive_vendor") to readable title case
    # ("Aggressive Vendor") so the rendered sev-line doesn't show underscores.
    # Also applies to "very_common" frequency, which renders "Very Common".
    cat = scam["category"].replace("_", " ").title()
    sev = scam["severity"].title()
    freq = scam["frequency"].replace("_", " ").title()
    parts = [
        f'## {scam["name"]}\n\n',
...
def city_chapter_md(data: dict) -> str:
    ...
    # Note the slug here can include a country suffix (e.g. "jaco-costa-rica")
    # that doesn't match the on-disk filename ("jaco.jpg"); _img_ref returns
    # None for such mismatched slugs, preserving the original "skip" behavior.
    city_ref = _img_ref(f"assets/cities/{slug}.jpg")
    if city_ref:
        alt = CITY_ALT_TEXT.get(slug, f"Stylized illustration of {city}")
        parts.append(f"![{alt}]({city_ref})\n\n")
...
def static_chapters() -> list[tuple[str, str]]:
    ...
    return [(f.name, _rewrite_md_assets(f.read_text())) for f in files]
...
def _inline_svg_images(svg_text: str, svg_dir: Path) -> str:
    """Inline every <image href> as a base64 data URI.

    rsvg-convert 2.62+ silently drops image refs (relative, absolute, AND
    HTTPS URLs) — without inlining we'd render a text-only cover. For
    HTTPS URLs pointing at the R2 mirror, prefer the local file if it
    still exists; otherwise fetch the bytes from the URL.
    """
    import base64
    import mimetypes
    ...
    def _inline(match: re.Match) -> str:
        ...
        if href.startswith(("http://", "https://")):
            ...
            local = (Path(href) if href.startswith("/") else svg_dir / href)
            if not local.exists():
                return match.group(0)
            data = local.read_bytes()
            mime = mimetypes.guess_type(local.name)[0] or "image/jpeg"
...
def render_cover() -> None:
    ...
    if jpg.exists() and jpg.stat().st_mtime >= svg.stat().st_mtime:
        return
    # Inline all <image href>s as base64 — rsvg-convert 2.62+ silently drops
    # relative/absolute/HTTPS image refs, so without this the cover renders
    # as a text-only gradient.
    svg_inlined = _inline_svg_images(svg.read_text(), svg.parent)
