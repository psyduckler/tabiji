...
#
# Outputs (written to book-cosmetic-surgery/build/):
#   - the-cosmetic-surgery-field-guide.epub  (Kindle ebook)
...
SRC_MS="$BOOK_DIR/manuscript/00-the-cosmetic-surgery-field-guide.md"
...
pandoc "$BUILD_DIR/manuscript-processed.md" \
  -o "$BUILD_DIR/the-cosmetic-surgery-field-guide.epub" \
  --epub-cover-image="$BUILD_DIR/kindle-cover.jpg" \
  --css="$EPUB_CSS" \
  --metadata title="The Cosmetic Surgery Field Guide" \
  --metadata subtitle="A Buyer-Protection Guide to Cosmetic Surgery and Hair Restoration Abroad" \
