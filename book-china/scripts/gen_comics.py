#!/usr/bin/env python3
"""
Generate the China book cover art via Wavespeed (Nano Banana Pro):
 - front cover (dramatic 2:3 portrait scene, Chinese scam in action)
 - back cover (2:3 portrait scene of Chinese atmosphere with upper
   negative space for cover-copy overlay)

Scam-comic assets for the 98 mainland-China scams already live on R2 at
img.tabiji.ai/scams/<slug>/scam-N.jpg and are downloaded into
book-china/assets/images/<slug>/NN.jpg by the initial setup step.
This script only needs to generate the two covers.

Usage:
    python3 book-china/scripts/gen_comics.py              # both covers
    python3 book-china/scripts/gen_comics.py --front-only
    python3 book-china/scripts/gen_comics.py --back-only
"""
...
STYLE_COMIC = (
    ...
    "palette with warm cream and muted ochre background, gentle brush-ink "
    "shading inspired by Chinese poetic-cartoon illustration. Matches a "
...
COVERS = [
    (
        ...
        (
            ...
            "depicting a mainland-China tourist scam in action near the gates of "
            "the Forbidden City in Beijing: a female tourist in a sun hat stands "
            "on the avenue in front of the ornate red-and-gold Tiananmen gate, "
            "looking slightly worried, while a smiling young Chinese woman in "
            "casual modern clothes holds up a small art portfolio and says "
            "'Free student gallery — just around the corner!' in a white "
            "speech bubble. In the middle distance a red taxi waits with its "
            "driver waving, and a red-pillared hutong alley branches off to "
            "the side. Sky is warm saffron-and-dusty-purple golden-hour. "
            ...
            "red-and-gold, muted teal shadows, cream sky. No book title, no "
    ...
    (
        ...
        (
            ...
            "a bustling Chinese old-town evening from a three-quarter overhead "
            "view: a narrow hutong or old-street lane with red paper lanterns "
            "strung overhead, warm glow from small food-stall braziers, a few "
            "silhouetted figures walking along the cobblestones, and the distant "
            "silhouette of a pagoda or temple roof against a twilight sky. "
            "Composition is atmospheric and calm, with substantial empty "
            "dusty-purple sky in the upper two-thirds of the frame to leave "
            "space for back-cover copy to be overlaid. Palette: pastel "
            "watercolor, warm red-lantern glow, hints of saffron and sage and "
            "indigo. No text, no watermark, no book title."
        ),
        "2:3",
    ),
]
