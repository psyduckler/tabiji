...
def strip_reddit_fragments(md: str) -> str:
    ...
    # Title-match: use a TIGHT character class that excludes internal quotes.
    # Using `.+?` with DOTALL here caused catastrophic backtracking on
    # descriptions with many internal apostrophes (e.g., Munich Marienplatz:
    # `r/Munich 'title' (2025) is a report.` followed by bullet-list sentences
    # containing `'deaf-mute petition'`, `'friendship bracelet'` etc. made
    # `.+?` extend across the entire description, consuming it.
    # Tradeoff: titles with internal apostrophes (e.g., `"I'm going to X"`)
    # are not stripped, but their descriptions survive. That's a safer failure.
    _TITLE = r"['\u2018\u2019\"][^'\u2018\u2019\"\n]{2,200}['\u2018\u2019\"]"

    md = re.sub(
        r"\s*r/\w+\s+" + _TITLE + r"(?=[\s(,;])"     # r/sub 'title'
    ...
    md = re.sub(
        r"\s*(?:per\s+|from\s+|via\s+)?r/\w+\s+" + _TITLE + r"(?=[\s(,;])"
        r"[^.\n]{0,400}\.",
        " ",
        md,
    )
