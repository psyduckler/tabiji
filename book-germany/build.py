...
# Rich alt text for German city chapter openers (screen-reader friendly).
CITY_ALT_TEXT: dict[str, str] = {
    "berlin": "Berlin — the Brandenburg Gate quadriga silhouette and the Fernsehturm television tower across Pariser Platz at dusk",
    "potsdam": "Potsdam — the Sanssouci palace terraces and formal baroque gardens in late-afternoon light",
    "hamburg": "Hamburg — the red-brick Speicherstadt warehouse district and the Elbphilharmonie glass sail above the harbor",
    "bremen": "Bremen — the Rathaus gable and the bronze Town Musicians of Bremen statue in the Marktplatz",
    "dresden": "Dresden — the Zwinger baroque palace colonnade and the Frauenkirche dome reflected in the Elbe at golden hour",
    "leipzig": "Leipzig — the Völkerschlachtdenkmal monument silhouette and the Augustusplatz opera house",
    "munich": "Munich — the twin onion-domed Frauenkirche towers and the Marienplatz Rathaus facade at blue hour",
    "nuremberg": "Nuremberg — the Kaiserburg castle walls and the timbered roofs of the Altstadt below",
    "frankfurt": "Frankfurt — the Römerberg timbered town-hall row and the modern banking skyline beyond",
    "stuttgart": "Stuttgart — the Schlossplatz palace and the Jubiläumssäule column in early-evening light",
    "cologne": "Cologne — the twin Gothic spires of the Kölner Dom above the Hohenzollern Bridge and the Rhine",
    "dusseldorf": "Düsseldorf — the Rheinturm television tower and the MedienHafen glass architecture at sunset",
    "heidelberg": "Heidelberg — the red-sandstone Schloss ruin above the Neckar and the Alte Brücke stone arches",
    "baden-baden": "Baden-Baden — the colonnaded Trinkhalle pump-room and the Kurhaus garden terraces",
    "rothenburg": "Rothenburg ob der Tauber — the half-timbered Plönlein fork and the red-tiled Altstadt rooftops",
    "fussen": "Füssen — Neuschwanstein castle rising from forested hills against the Alps at dawn",
}
...
def city_chapter_md(data: dict) -> str:
    ...
    if intro_path.exists():
        ...
        parts.append(
            ...
            f"nearest Polizei (police) station, one safe recommendation.\n\n"
        )
    parts.append(
...
def _normalize_currency(md: str) -> str:
    """Germany uses euro (€), which is well-supported in Arial Unicode MS, so
    there's no symbol replacement needed. This function only sweeps bare
    hyphens in numeric ranges to en-dashes for currency contexts (€, EUR,
    euro, euros, $), since Pandoc's +smart extension doesn't touch `12-20 €`.

    Kept as a named function so the rest of the build pipeline doesn't need
    ...
    # Trailing-currency-word form: "12-20 €", "50-60 EUR"
    md = re.sub(
        r"(\b[\d,.]+)-([\d,.]+\s*(?:EUR|euro|euros|€))",
        r"\1–\2",
        md,
    )
    # Leading-currency form: "€50-60"
    md = re.sub(r"(€[\d,.]+)-(€?[\d,.]+)", r"\1–\2", md)
    # USD ranges (for comparison conversions)
    md = re.sub(r"(\$[\d,.]+)-(\$?[\d,.]+)", r"\1–\2", md)
...
def assemble_markdown() -> str:
    ...
    for fname, content in static_chapters():
        ...
        parts.append("\n\n")
    return polish_markdown(_normalize_currency("".join(parts)))
...
def build_epub(md: str) -> Path:
    ...
    out_name = CONFIG.get("output_filename", "germany-scams")
